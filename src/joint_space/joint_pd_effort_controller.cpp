  ...
  const rclcpp_lifecycle::State &)
{
  ...
  reference_sub_ =
    get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
      ...
      rclcpp::SystemDefaultsQoS(),
      std::bind(
        &JointPDEffortController::referenceCallback,
        this,
        std::placeholders::_1));

  RCLCPP_INFO(
    get_node()->get_logger(),
    "JointPDEffortController subscribed to ~/reference.");

  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured JointPDEffortController with %zu joints.",
    n);
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (hold_initial_position_ || q_des_.empty()) {
    ...
    RCLCPP_INFO(
      get_node()->get_logger(),
      "JointPDEffortController holding initial joint position.");
  }
...
void JointPDEffortController::referenceCallback(
  const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
  ...
  if (msg->points.size() > 1) {
    RCLCPP_WARN(
      get_node()->get_logger(),
      "JointPDEffortController received %zu trajectory points. "
...
bool JointPDEffortController::validateTrajectoryMessage(
...
bool JointPDEffortController::acceptSinglePointCommand(
  const trajectory_msgs::msg::JointTrajectory & trajectory)
{
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "JointPDEffortController accepted single-point reference with %zu joints.",
    n);

  return true;
}

bool JointPDEffortController::readStateInterfaces()
{
  const auto n = joint_names_.size();
...
bool JointPDEffortController::computePDEffortCommand()
{
  const auto n = joint_names_.size();
...
bool JointPDEffortController::writeCommandInterfaces()
{
